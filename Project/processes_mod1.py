    ...
    def Transcript(self, DNA):
        ...
        if DNA[j*3:j*3+3] == StartCodon:
            while DNA[j*3:j*3+3] not in StopCodonDict:
                messRNA_sequence += DNA[j*3:j*3+3]
                j = j+1
            messRNA_sequence += DNA[j*3:j*3+3]
            messRNA_sequence = messRNA_sequence.replace('T','U')
        else:
        ...
        messRNA_ID = 1 #extract from Database
        mol.MRNA.sequence = messRNA_sequence

        print mol.MRNA.sequence
